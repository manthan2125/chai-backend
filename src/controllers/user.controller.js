...
const registerUser = asyncHandler(async (req, res) => {
  ...
  */
    ...
    const avatar = await uploadOnCloudinary(avatarLocalPath);    // it is returning response - but response.url(avatar.url) constains the url
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    const avatarLocalPath = req.file?.path;       // mein avatar update krne ke liye ek nhyi avatar file bhejunga jo req. file mein jaegi
    if (!avatarLocalPath) {
        throw new ApiError(400, "Avatar file is missing")
    }

    // Find current user
    const existingUser = await userModel.findById(req.user._id).select("-password -refreshToken");
    if (!existingUser) {
    ...
    const avatar = await uploadOnCloudinary(avatarLocalPath)  // this is updated uplaodedavatar:  is an object returned by uploadOnCloudinary 
...
const getUserChannelProfile = asyncHandler(async(req, res) => {
    ...
    const channel = await userModel.aggregate([
        ...
        {
            $lookup : {   // calc subscribers seeing channels
                from: "subscriptions",
                localField: "_id",
        ...
        {
            $lookup: {       // to whom this user has subscribedTo
                from: "subscriptions",
                localField: "_id",
        ...
        {
            $addFields: {     // Will add these fields in usermodel
                subscribersCount: {
                    $size: "$subscribers"
                },
    ...
    if(!channel?.length){   // if channel ki length 0 hai 
